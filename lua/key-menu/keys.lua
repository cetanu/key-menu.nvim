...
---Get built-in keymaps for a mode.
---@param mode string
---@return table[]
function M.get_builtin_keymap(mode)
  local ok, builtins = pcall(require, 'km-builtins')
  if not ok then
    -- Fallback or empty if module not found.
    -- Warning: this might break if km-builtins is expected.
    -- Assuming it returns a list of mappings.
    return {}
  end
  local keymap = M.normalize_keymap(builtins)
